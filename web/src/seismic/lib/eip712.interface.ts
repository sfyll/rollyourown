 ...
 * wrapper tx object.
 */
export function createEIP712TypesNoBodyStarknet(name: string, spec: { name: string; type: string }[]) {
  return {
...
export function createStarknetEIP712DomainType(name: string) {
  return {
    ...
    chainId:
      typeof process.env.CHAIN_ID === "string"
        ? shortString.encodeShortString(process.env.CHAIN_ID)
        : Number(process.env.CHAIN_ID) || shortString.encodeShortString("KATANA"),
  };
}
