...
export const calculateSlippage = (market: Market, tradeAmount: number, tradeDirection: TradeDirection) => {
  const k = market.cash * market.quantity;
  const currentPrice = market.cash / market.quantity;

  const quantity =
    tradeDirection === TradeDirection.Buy ? market.quantity - tradeAmount : market.quantity + tradeAmount;
...
export const calculateMaxQuantity = (market: Market, maxCash: number) => {
  const k = market.cash * market.quantity;
  const maxQuantity = market.quantity - k / (Number(market.cash) + maxCash * SCALING_FACTOR);
